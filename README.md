...
预测研究页：<http://127.0.0.1:8001/microstructure/research/>。当前可切换研究
主动成交失衡和成交强度与未来5分钟收益的关系；成交强度定义为当前1分钟成交额
相对前60个连续完整分钟成交额中位数的倍数。未来收益只在当前分钟至5分钟后六根
K线严格连续且全部收盘时生成；研究按时间前70%发现、后30%验证，并使用发现集
十分位边界统计两段的样本数、平均未来收益和上涨比例。迁移后可回填既有分钟数据：
