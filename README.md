...
每轮消息提供默认收起的“查看调用链”按钮。展开后按处理阶段显示已保存的快照、
上下文概况、工具参数与结果、最终状态和调用用量；运行中的展开面板自动刷新，
收起时不额外请求调用记录。旧对话同样可以查看，原始数据依据保留在面板内。
这不是模型内部思考，也不是完整逐次模型请求轨迹；同参数工具缓存复用不重复计数。
模型返回损坏的工具参数、重复报告或不合约定的报告结构时，自动修复最多两次，
且仍受本轮模型调用总上限约束。修复会移除异常回复及其反馈，保留之前成功的工具结果，
重新请求一份合法报告；旧 checkpoint 中的此类异常也会在再次请求模型前处理。
调用链记录修复原因、次数和时间；失败轮也保留已收到的 token 用量，达到上限后给出明确提示。
