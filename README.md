...
Market Evidence Lab 是一个面向市场证据工作流的 Django 单体项目。当前完成第 5 阶段“市场异常巡检闭环 V1”：在采集、数据质量和自动调度能力之上，从 ETHUSDT 1d K线识别符合固定V1规则的异常日期候选，并保存运行、规则与指标快照。
...
同一天可以命中多个信号，但一次扫描只保存一条候选发现。扫描成功不表示“通过”，只表示程序正常完成；没有异常时异常日期数为0。市场异常候选不包含原因解释、未来收益、相似样本、正式研究案例或预测结论。
...
当前没有实现通用 Cron 平台、Celery、Redis、消息队列、APScheduler、实时 WebSocket、自动补采或修复、技术指标、K线图、行情原因分析、未来收益、历史相似样本、研究案例、AI报告、人工反馈、登录权限或 Django Admin 页面。没有采集 1m、OI、Funding、订单簿、成交明细、新闻或链上数据。市场异常巡检尚未接入自动调度。
