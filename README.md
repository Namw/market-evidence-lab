...
Market Evidence Lab 是一个面向市场证据工作流的 Django 单体项目。当前完成第 6 阶段“研究案例 V1”：在市场异常巡检候选之上，由人工选择异常日建立正式研究案例，并冻结创建时的异常信号与行情快照。
...
案例保存来源 finding（删除保护）、案例标题、市场身份、UTC事件时间、异常信号、OHLC、成交量、涨跌幅、振幅及计算快照。列表和详情优先展示中文异常类型与人类可读摘要；英文枚举、精确参数和 JSON 位于折叠明细。价格证据、衍生品证据、新闻证据、历史类似与 AI 报告目前仅展示“待建设”空状态，不包含虚假内容或对应模型。
...
当前没有实现通用 Cron 平台、Celery、Redis、消息队列、APScheduler、实时 WebSocket、自动补采或修复、技术指标、K线图、行情原因分析、未来收益、历史相似样本、研究案例证据模型、AI报告、人工反馈、登录权限或 Django Admin 页面。没有采集 1m、OI、Funding、订单簿、成交明细、新闻或链上数据。市场异常巡检尚未接入自动调度。
